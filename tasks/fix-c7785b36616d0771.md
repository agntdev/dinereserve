# fix-c7785b36616d0771 — `reminder_sent_at` missing from `BookingRow` type and mapper

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 DRSV

The `bookings` table schema in `db/schema.sql:29` includes a `reminder_sent_at` column, and `reminders.ts:166-176` writes to it via `markReminderSent()`. However, the `BookingRow` interface (`src/db/types.ts:19-32`) omits this field, and the `mapBookingRow` mapper (`src/db/repository.ts:132-153`) does not select or map it. Any code that reads bookings through the repository (`getById`, `getByRefCode`, `listByDate`, `listOverlapping`, `create`) will silently lose this value, even though the column is populated in the database.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-c7785b36616d0771.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-c7785b36616d0771.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
