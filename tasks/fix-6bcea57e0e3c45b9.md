# fix-6bcea57e0e3c45b9 — listDueBookings query missing lower time bound

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 DRSV

The SQL query in `src/reminders.ts:146-156` filters bookings with `start_dt <= NOW() + ($1::int * INTERVAL '1 minute')` but has **no lower bound** (e.g. `start_dt > NOW()`). This means bookings whose `start_dt` is already in the past still match the query as long as they are within the offset window forward. On bot startup or after a polling gap, this can send "reminder" messages for bookings that have already started — a confusing user experience.

Fix: add `AND start_dt > NOW()` to the `WHERE` clause.

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-6bcea57e0e3c45b9.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-6bcea57e0e3c45b9.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
