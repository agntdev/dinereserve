# E1T6-fix — Debug /reserve command failure

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 DRSV

Investigate and resolve why /reserve command triggers error:
- Check command registration in T01
- Validate flow transitions between E1T2 and E1T6
- Add proper error boundaries and fallbacks
- Ensure proper state management during reservation flow

## Skills

**Skills:** telegram-bot-basics, debugging
...
If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/E1T6-fix.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/E1T6-fix.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
