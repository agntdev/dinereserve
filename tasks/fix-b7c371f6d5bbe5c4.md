# fix-b7c371f6d5bbe5c4 — Orphaned restaurant_tables rows when re-running setup with fewer tables (MEDIUM)

**Weight:** 0.0000 (share of project budget)
**Reward:** 0 DRSV

In `persistSetup()` at `src/admin/setup.ts:230-238`, the function iterates from i=1 to `setupTableCount` and issues `INSERT … ON CONFLICT (id) DO UPDATE`. It never deletes tables with IDs beyond the new count. If a first setup creates tables t1–t6 and a subsequent setup reconfigures to 4 tables, t5 and t6 remain in the DB as orphaned rows. These stale tables can silently affect any future code that reads from `restaurant_tables` (e.g. seed, capacity-calculation).

## Dialog tests

If this task adds or changes user-facing bot behavior, author its dialog tests as a `BotSpec` JSON array in its OWN file `tests/specs/fix-b7c371f6d5bbe5c4.json`. NEVER edit or append to a shared `tests/specs.json` — concurrent feature PRs would conflict on it. The tests-gate globs and merges all `tests/specs/*.json`.

If this task adds a bot command, declare it in its OWN file `tests/commands/fix-b7c371f6d5bbe5c4.json` (a JSON array of command strings, e.g. `["/start"]`). NEVER edit or append to a shared `tests/commands.json` — same conflict reason. The tests-gate globs, merges + de-duplicates all `tests/commands/*.json`.
